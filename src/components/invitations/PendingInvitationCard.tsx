...
import { Clock, Users, MessageCircle, X, MapPin, Calendar, Coins, Zap } from 'lucide-react';
              ...
              <div className="space-y-1">
                {invitation.stakes_tokens! > 0 && (
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span>{invitation.stakes_tokens} Regular Tokens (Escrowed)</span>
                  </div>
                )}
                {invitation.stakes_premium_tokens! > 0 && (
                  <div className="flex items-center gap-2 text-sm">
                    <Coins className="h-3 w-3 text-purple-500" />
                    <span>{invitation.stakes_premium_tokens} Premium Tokens (Escrowed)</span>
                  </div>
                )}
                <p className="text-xs text-orange-700 mt-1">
                  {invitation.invitation_category === 'match' ? 'Stakes are held in escrow until match completion' : 'Entry fee for premium session'}
                </p>
              </div>
            </div>
          )}
