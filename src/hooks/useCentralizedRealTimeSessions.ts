...
interface UseCentralizedRealTimeSessionsOptions {
  enabled?: boolean;
...
export function useCentralizedRealTimeSessions(
  activeTab: string, 
  userId?: string,
  options: UseCentralizedRealTimeSessionsOptions = {}
) {
  const { user } = useAuth();
  const {
    enabled = true,
  ...
  // Set up centralized real-time subscriptions
  useEffect(() => {
    if (!enabled || !effectiveUserId) return;

    console.log('🔄 Setting up centralized realtime subscriptions for sessions');
    ...
    // Session participants table subscription
    const participantsCallback = () => {
      console.log('📨 Session participants table changed, refetching...');
      fetchSessions();
    };

    // Subscribe through centralized manager
    const sessionsSubscriptionId = realtimeManager.subscribe(
      {
        table: 'sessions',
        event: '*',
        schema: 'public'
    ...
    const participantsSubscriptionId = realtimeManager.subscribe(
      {
    ...
    return () => {
      console.log('🧹 Cleaning up centralized subscriptions');
      realtimeManager.unsubscribe(sessionsSubscriptionId, sessionsCallback);
      realtimeManager.unsubscribe(participantsSubscriptionId, participantsCallback);
    };
  }, [effectiveUserId, enabled, fetchSessions]);
