...
DO $$
BEGIN
  -- Add subscription tier check constraint
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'clubs_subscription_tier_check') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'clubs_court_count_positive') THEN
    ALTER TABLE clubs ADD CONSTRAINT clubs_court_count_positive 
      CHECK (court_count > 0);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'clubs_coach_slots_positive') THEN
    ALTER TABLE clubs ADD CONSTRAINT clubs_coach_slots_positive 
      CHECK (coach_slots > 0);
  END IF;
END
$$;
