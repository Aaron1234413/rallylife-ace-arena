-- Fix HP activities insertion in complete_session function
...
BEGIN
  ...
  ELSE
    ...
    LOOP
      ...
      -- Get current HP before updating
      SELECT current_hp INTO current_hp_value
      FROM public.player_hp
      WHERE player_id = participant_record.user_id;
      
      -- If no HP record exists, initialize it
      IF current_hp_value IS NULL THEN
        PERFORM public.initialize_player_hp(participant_record.user_id);
        current_hp_value := 100;
      END IF;
      
      -- Calculate new HP value
      new_hp := GREATEST(20, current_hp_value - hp_cost);
      ...
      -- Log HP consumption with proper hp_before and hp_after values
      INSERT INTO public.hp_activities (player_id, activity_type, hp_change, hp_before, hp_after, description)
      VALUES (participant_record.user_id, 'training', -hp_cost, current_hp_value, new_hp,
