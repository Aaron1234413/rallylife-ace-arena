...
-- Remove coach avatar initialization from signup process
-- This prevents the constraint violation error during coach registration

-- Update handle_new_user function to exclude coach avatar initialization
...
BEGIN
  ...
  -- Initialize CRP, CXP, and CTK for coaches (removed avatar initialization)
  IF COALESCE((NEW.raw_user_meta_data ->> 'role')::public.user_role, 'player'::public.user_role) = 'coach' THEN
    ...
    -- Removed: PERFORM public.initialize_coach_avatar(NEW.id);
  END IF;
