-- Phase 1: Enhanced Session Cancellation and Refund System (Simplified)

-- 1. Update tables schema to support cancellation tracking
...
-- 2. Create function to calculate refund percentage based on timing for open_sessions
CREATE OR REPLACE FUNCTION public.calculate_refund_percentage_open_sessions(
  scheduled_date DATE,
  start_time TIME,
...
BEGIN
  ...
  refund_percentage := public.calculate_refund_percentage_open_sessions(
...
-- 5. Create simple session cancellation function for basic sessions table
...
-- 6. Update status constraints to include new statuses
ALTER TABLE public.open_sessions 
...
CREATE POLICY "Users can view cancelled open sessions they created"
ON public.open_sessions
FOR SELECT
USING (
  status IN ('cancelled', 'expired') AND creator_id = auth.uid()
);