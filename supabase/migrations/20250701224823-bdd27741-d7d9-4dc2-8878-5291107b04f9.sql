-- Phase 1: Foundation & Database Alignment Migration
...
-- 3. Standardize the invitation system by adding a linking field for social play sessions
-- The match_session_id in match_invitations can also reference social_play_sessions when invitation_category = 'social_play'
-- We'll add a proper polymorphic reference system

-- 4. Create indexes for better performance on frequently queried columns
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_match_invitations_invitee_status 
ON public.match_invitations(invitee_id, status);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_match_invitations_inviter_status 
ON public.match_invitations(inviter_id, status);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_match_invitations_category_status 
ON public.match_invitations(invitation_category, status);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_social_play_participants_session_user 
ON public.social_play_participants(session_id, user_id);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_social_play_sessions_creator_status 
ON public.social_play_sessions(created_by, status);

-- 5. Add proper constraints to ensure data consistency
...
-- 6. Update RLS policies to ensure proper cross-table access
...
-- 7. Clean up any orphaned data (optional safety measure)
...
-- 8. Add helpful comment for future reference
