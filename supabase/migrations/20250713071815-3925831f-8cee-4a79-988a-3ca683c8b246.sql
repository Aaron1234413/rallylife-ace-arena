-- Drop all existing versions of complete_session_unified to avoid conflicts
DROP FUNCTION IF EXISTS public.complete_session_unified(uuid, uuid, text[], jsonb);
DROP FUNCTION IF EXISTS public.complete_session_unified(uuid, uuid, jsonb, jsonb);
...
BEGIN
  ...
  SELECT COUNT(*), COALESCE(SUM(stakes_contributed), 0)
  INTO participant_count, total_stakes
  FROM session_participants sp
  WHERE sp.session_id = session_id_param 
    AND sp.status = 'joined';
  
  RAISE LOG 'complete_session_unified: Found % participants with % total stakes', 
    participant_count, total_stakes;
  ...
  FOR participant_record IN 
    SELECT * FROM session_participants sp
    WHERE sp.session_id = session_id_param 
      AND sp.status = 'joined'
  LOOP
