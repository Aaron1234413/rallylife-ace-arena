...
DROP POLICY IF EXISTS "Participants: manage participation" ON public.match_participants;
...
-- Drop policies that might already exist from previous attempts
...
DROP POLICY IF EXISTS "Users can join match sessions as participants" ON public.match_participants;
DROP POLICY IF EXISTS "Users can update their own participation" ON public.match_participants;

-- Now we can safely drop the helper functions that might cause recursion
