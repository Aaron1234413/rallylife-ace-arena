-- Drop existing complete_session functions to resolve overloading conflict
DROP FUNCTION IF EXISTS public.complete_session(UUID, UUID, TEXT, INTEGER);
DROP FUNCTION IF EXISTS public.complete_session(UUID, UUID, INTEGER);
DROP FUNCTION IF EXISTS public.complete_session(UUID, UUID, TEXT);

-- Create unified complete_session function
