-- Fix the join_session function to use correct session_participants column names
...
BEGIN
  ...
  -- Start transaction
  BEGIN
    -- Add user to session participants using correct column names
    INSERT INTO public.session_participants (
      session_id, user_id, joined_at, status, stakes_contributed,
      tokens_paid, money_paid, payment_status, attendance_status
    )
    VALUES (
      session_id_param, user_id_param, now(), 'joined', session_record.stakes_amount,
      session_record.stakes_amount, 0, 'paid', 'registered'
    );
