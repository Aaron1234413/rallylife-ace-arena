-- Fix currency validation in book_club_service function - price_usd is already in cents
...
DECLARE
  ...
  total_cost_cents INTEGER;  -- Changed from total_cost_usd to be clearer
BEGIN
  ...
  total_cost_cents := service_record.price_usd;  -- price_usd is already in cents
  
  IF tokens_to_use > total_cost_tokens THEN
    RAISE EXCEPTION 'Token amount exceeds service price';
  END IF;
  
  IF cash_amount_cents > total_cost_cents THEN
  ...
  VALUES (
    ...
    CASE WHEN tokens_to_use > 0 AND cash_amount_cents > 0 THEN 'hybrid'
         ...
         ELSE 'cash' END,
    tokens_to_use, cash_amount_cents, total_cost_tokens, total_cost_cents
  )
