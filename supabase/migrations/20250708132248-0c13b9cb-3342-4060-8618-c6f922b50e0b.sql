-- Phase 1: Backend Foundation & Fixes (Data cleanup first)
...
-- 1.3 Clean up data FIRST, then update constraints
-- Update all references to 'free' tier BEFORE changing constraint
UPDATE public.club_subscriptions 
SET tier_id = 'community' 
WHERE tier_id = 'free';
...
DROP CONSTRAINT IF EXISTS clubs_subscription_tier_check;

ALTER TABLE public.clubs 
ADD CONSTRAINT clubs_subscription_tier_check 
CHECK (subscription_tier = ANY (ARRAY['community'::text, 'core'::text, 'plus'::text, 'pro'::text]));

-- Now safely remove 'Free' tier from subscription_tiers
