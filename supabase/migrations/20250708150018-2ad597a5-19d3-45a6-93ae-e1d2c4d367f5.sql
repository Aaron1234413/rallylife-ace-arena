-- Phase 3: Session Participants and Unified Data Flow (Fixed)
...
DROP POLICY IF EXISTS "Users can join sessions they have access to" ON public.session_participants;
...
DROP POLICY IF EXISTS "Users can view session participants for accessible sessions" ON public.session_participants;
...
-- Create updated_at trigger for session_participants if not exists
...
-- Add constraint for club sessions requiring session_source
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.check_constraints 
    WHERE constraint_name = 'sessions_club_source_check'
  ) THEN
