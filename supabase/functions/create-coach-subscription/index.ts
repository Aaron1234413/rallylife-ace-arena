...
const logStep = (step: string, details?: any) => {
  ...
  console.log(`[CREATE-COACH-SUBSCRIPTION] ${step}${detailsStr}`);
};
  ...
  try {
    ...
    if (customers.data.length > 0) {
      ...
      const customer = await stripe.customers.create({
        email: user.email,
        metadata: { user_id: user.id, subscription_type: 'coach' }
      });
    ...
    const priceData = {
      premium: { amount: 799, name: "Premium Coach Plan" }, // $7.99
      pro: { amount: 1499, name: "Pro Coach Plan" } // $14.99
    };
      ...
      line_items: [
        {
          price_data: {
            ...
            product_data: { 
              name: price.name,
              description: `${monthly_token_allocation} tokens/month, manage up to ${max_clubs_allowed} clubs`
            },
    ...
    logStep("ERROR in create-coach-subscription", { message: errorMessage });
